    ...
    var body: some View {
            ...
            VStack(spacing: 20) {                
                ...
                if $habit.hasDailyReminder.wrappedValue {
                    ZStack {
                        VStack(spacing: 20) {
                            TimePicker(habit: $habit, title: "Set a time")
                        }  
                        .frame(width: .infinity)       
                    }
                    .frame(width: .infinity, height: 90)
...
#Preview {
    ...
    return HabitSettingsView(habit: .constant(Habit(habitName: "Placeholder Habit Name", habitDescription: description, habitType: "Start", targetDays: 14)))
}
